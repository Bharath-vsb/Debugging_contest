  ...
  return (
    <div className="admin">
      ...
      <div className="admin__content">
        ...
        {/* ════════════════════ SCORES TAB ════════════════════ */}
        {tab === 'scores' && (
          <ScoresPanel dashboard={dashboard} onRefresh={fetchDashboard} />
        )}
      </div>
    </div>
  );
...
function QuestionsPanel({ questions, qFilter, setQFilter, onDelete }) {
  ...
  return (
    <div className="admin-panel">
        ...
        {questions.map(q => (
          <div key={q._id} className="q-card">
            ...
            <div className="q-card__actions">
              <button className="btn btn--secondary btn--sm" onClick={() => setViewingQuestion(q._id)}>View</button>
      ...
      {/* Modal */}
      {viewingQuestion && (
        <QuestionDetailModal questionId={viewingQuestion} onClose={() => setViewingQuestion(null)} />
      )}
    </div>
  );
}
...
/* ══════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════ */
function QuestionDetailModal({ questionId, onClose }) {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        if (data.success) setData(data.question);
      } catch (err) {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-meta">
          <span>Round {data.round}</span> • <span>{data.language}</span>
        </div>

        <div className="modal-section">
          <h3>Correct Code</h3>
          <pre className="code-block">{data.correctCode}</pre>
        </div>

        <div className="modal-section">
          <h3>Generated Test Cases ({data.testCases.length})</h3>
        ...
        <div className="modal-section">
          <h3>Generated Buggy Code (Preview)</h3>
          <pre className="code-block">{data.buggyCode}</pre>
        </div>
...
/* ══════════════════════════════════════════════════════════
   SCORES PANEL
   ══════════════════════════════════════════════════════════ */
function ScoresPanel({ dashboard, onRefresh }) {
  ...
  const deleteStudent = async (studentId, studentName) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Refresh the dashboard data without reloading page to prevent logout
