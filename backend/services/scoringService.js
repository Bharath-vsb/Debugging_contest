...
async function scoreSubmission(submittedCode, language, testCases) {
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    // Normalize: split by whitespace and join with single space
    const actualTokens = actualRaw.trim().split(/\s+/);
    const expectedTokens = expectedRaw.trim().split(/\s+/);

    // Check if tokens match
    let passed = false;
    if (actualTokens.length === expectedTokens.length) {
      passed = actualTokens.every((token, index) => token === expectedTokens[index]);
    }
    ...
    testResults.push({
      testIndex: i,
      passed,
      actualOutput: actualRaw.trim(), // Store trimmed raw for display
