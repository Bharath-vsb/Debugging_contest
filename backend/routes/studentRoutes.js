...
router.get('/:rollNumber/questions/:round', async (req, res) => {
  try {
    ...
    // Fetch questions – EXCLUDE correctCode, BUT INCLUDE testCases
    const questions = await Question.find({ _id: { $in: questionIds } })
      .select('-correctCode'); // correctCode is safely hidden, but testCases are now sent
    ...
    const enriched = questions.map(q => ({
      ...
      testCases: q.testCases, // Now exposed to frontend
...
router.post('/:rollNumber/submit', async (req, res) => {
  try {
    ...
    let timeTaken = 0;
    if (startedAt) {
      timeTaken = Math.floor((submittedAt.getTime() - new Date(startedAt).getTime()) / 1000); // Seconds
    }
...
/* ══════════════════════════════════════════════════════════
   POST /api/student/:rollNumber/run
   Run code against test cases WITHOUT submitting/saving score.
   Used for "Run Code" / "Test" button.
   Body: { questionId, code }
   ══════════════════════════════════════════════════════════ */
router.post('/:rollNumber/run', async (req, res) => {
  try {
    ...
    // Reuse scoreSubmission logic but ignore the 'score' result
    // We only care about testResults
    const result = await scoreSubmission(code, question.language, question.testCases);
...
/* ══════════════════════════════════════════════════════════
   POST /api/student/:rollNumber/end-contest
   Manually ends the contest for the student.
   Calculates final score = (Sum of Round Scores) + Time Bonus.
   Time Bonus = (Max Contest Time - Actual Time Taken) * 0.5 points/min (Example)
   Max Contest Time = 90 mins (3 rounds * 30)
   Actual Time = Now - Round 1 Start Time
   ══════════════════════════════════════════════════════════ */
router.post('/:rollNumber/end-contest', async (req, res) => {
  try {
    const { rollNumber } = req.params;
    const student = await Student.findOne({ rollNumber });
    ...
    if (student.isContestEnded) {
      return res.json({
        success: true,
        message: 'Contest already ended',
        finalTotalScore: student.finalTotalScore,
        finalTimeBonus: student.finalTimeBonus
      });
    }

    // 1. Mark as ended
    student.isContestEnded = true;
    student.contestEndedAt = new Date();
    student.currentRound = 4; // Ensure no more rounds can be started

    // 2. Calculate Base Score (Sum of round scores)
    const r1 = student.scores.round1 || 0;
    const r2 = student.scores.round2 || 0;
    const r3 = student.scores.round3 || 0;
    const baseScore = r1 + r2 + r3;

    // 4. Final Score (No Time Bonus)
    const totalScore = baseScore;

    student.finalTimeBonus = 0;
    student.finalTotalScore = totalScore;
    ...
    return res.json({
      ...
      durationMinutes: student.roundStartedAt['1']
        ? Math.floor((new Date() - new Date(student.roundStartedAt['1'])) / 60000)
        : 0
    });
